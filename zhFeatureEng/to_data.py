...
def to_periods_test_data(data, last):
    ...
    for i in range(30):
        for j in range(7):
            A_test.loc[2976 - 672*(i + 1) : 2976 - 672*i, ('last_'+str(j + 1)+'_week_mean')] = last.loc[end - (j + 6 + i)*gap : end - (j + 5 + i)*gap, ( 'amount')].mean()
            A_test.loc[2976 - 672*(i + 1) : 2976 - 672*i, ('last_'+str(j + 1)+'_week_std')] = last.loc[end - (j + 6 + i)*gap : end - (j + 5 + i)*gap, ( 'amount')].std(ddof=0)
            A_test.loc[2976 - 672*(i + 1) : 2976 - 672*i, ('last_'+str(j + 1)+'_week_skew')] = last.loc[end - (j + 6 + i)*gap : end - (j + 5 + i)*gap, ( 'amount')].skew()
